...
$(function () {
    ...
    //     e.preventDefault();
    //     var no_of_rows = $('.sales_rows').length;
    //     $('#sales-log').append(
    //         '<tr class="sales_rows" id="sales_row_' + no_of_rows + '">' +
    //         '<td>' + '<select class="form-control" name="billsItem[]">'+
    //         '@foreach($billItems as $item)'+
    //         '<option value="{{$item->id}}"> {{$item->item_name}} </option>'+
    //         '@endforeach'+
    //         '</select> ' + '</td>' +
    //         '<td>' + '<input name="quantity[]" class=form-control placeholder="Item Quantity">' + '</td>' +
    //         '<td>' + '<input name="amount[]" class=form-control placeholder="Item Amount">' + '</td>' +
    //         '<td>' + '<input name="total[]" class=form-control placeholder="Item Total">' + '</td>' +
    //         '<td>' + '<button class="btn btn-danger" onclick="$(this).parents()[1].remove()">Remove</button>' + '</td>' +
    //         '</tr>'
    //     )
    // });
...
// set url for BIll items
$('.set-add-bil-item-url').on('click', function () {
    // $('#itemName').attr("value", '');
    ...
    var url = $(this).data('href');
    $('#addBillItemForm').attr("action", url);
});

//prepopulate Bill item to edit, change the url of form action
$('.update-bill-item').on('click', function (e) {
    e.preventDefault();
    var url = $(this).data('href');
    var item = $(this).data('item');
    $('#addBillItemForm').attr("action", url);
    $('#itemName').attr("value", item['item_name']);
    $('#expenseCategory').attr("value", item['expense_category']);
    $('#price').attr("value", item['price']);
    $('#itemCategory').attr("value", item['item_category']);
    $('#description').attr("value", item['description']);
    $('#drugCategory').attr("value", item['drug_category']);
    $('#weight').attr("value", item['weight']);
    $('#unit').attr("value", item['unit']);
    $('#feedCategory').attr("value", item['feed_category']);
});

// set url for farm items
$('.set-additem-url').on('click', function () {
    // $('#itemName').attr("value", '');
    // $('#addItemForm').trigger("reset");
    var token = $(this).data('token');
    $(':input').attr("value", '');//reset all input
...
//prepopulate item to edit, change the url of form action
...
$('.update-customer').on('click', function (e) {
    ...
    $('#customerForm').attr("action", url);
    $('#name').attr("value", customer['name']);
    $('#email').attr("value", customer['email']);
    $('#residence').attr("value", customer['residence']);
    $('#contact').attr("value", customer['contact']);
});
...
$('.update-penhouse-stocking-prepopulate').on('click', function (e) {
    ...
    console.log(penstocking);
    console.log(">>>>>>>>>>>>>>>>");
    $('#stockingForm').attr("action", url);
    // $('#birdType').attr("value", penstocking['type_of_bird']);
    // $('#penhouse').attr("value", penstocking['pen_house_id']);
    // $('#numberOfStock').attr("value", penstocking['number_of_stock']);
    // $('#batchId').attr("value", penstocking['batch_id']);
    // $('#dateDue').attr("value", penstocking['farm_payable']['date_due']);
    // $('#amount').attr("value", amount);
    // if (penstocking['farm_payable']['date_due'] != null) {
    //     $('[name="paid"]').attr('value', "YES");
    // }
    // else {
    //     $('[name="paid"]').attr('value', "NO");
    // }
});
...
// this function also set the amount to pay in the mpayment modal
...
//set url to delete customer
$(".delete-customer-modal").click(function () {
    var url = $(this).data("href");
    var newurl = $('.delete-feed-record').data('href', url); //setter

});

//set url to delete bill item
$(".delete-bill-item-modal").click(function () {
    var url = $(this).data("href");
    var newurl = $('.delete-bill-item').data('href', url); //setter
    // var a = $('.delete-item').data('href'); //getter

});

//set url to delete item
$(".delete-item-modal").click(function () {
    var url = $(this).data("href");
    var newurl = $('.delete-item').data('href', url); //setter
    // var a = $('.delete-item').data('href'); //getter
    ...
    $.ajax(
        {
            ...
            success: function (data) {
                ...
                console.log(json);
                if (json['status'] == "1") {
                    $('table#customerTable tr#' + json['id']).remove();
                    console.log("it Work");
                }
                else {
                    alert("This user cannot be deleted, User may have been used in other transaction");
                    console.log("Failed");
                }
            },
            error: function (data, textStatus, errorThrown) {
    ...
    $.ajax(
        {
            ...
            success: function (data) {
                var json = JSON.parse(data);
                if (json['status'] == "1") {
                    $('table#itemsTable tr#' + json['id']).remove();
                    console.log("it Work");
                }
                else {
                    alert("This item cannot be deleted, Item may have been used in other transaction");
                    console.log("Failed");
                }
            },
            error: function (data, textStatus, errorThrown) {
...
//delete a bill item
$(".delete-bill-item").click(function () {
    var url = $(this).data("href");
    console.log("url " + url);
    var token = $(this).data("token");
    $.ajax(
        {
            ...
            success: function (data) {
                var json = JSON.parse(data);
                if (json['status'] == "1") {
                    $('table#billItemsTable tr#' + json['id']).remove();
                    console.log("it Work");
                }
                else {
                    alert("This item cannot be deleted, Item may have been used in other transaction");
